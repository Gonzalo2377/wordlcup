...
/* ---- premium plan helpers (persisted; Stripe Payment Links) ---- */
function getPlan(){ try { return localStorage.getItem('mv_plan') || 'free'; } catch(e){ return 'free'; } }
...
function startCheckout(tier, setPlan){
    const u = stripeUrl(tier);
    if (u && !/PASTE|^#?$/.test(u)) { window.location.href = u; }   // real Stripe Payment Link
    else { setPlanLS(tier); if (setPlan) setPlan(tier); }            // demo fallback (no link configured yet)
}
